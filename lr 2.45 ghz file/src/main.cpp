 ...
 * @version 16.2 (API Fix)
 * @date 2024-08-01
 *
 * @details
 * Этот код реализует надежную передачу файла с гарантированной проверкой целостности.
 * ВЕРСИЯ 16.2: Убран вызов защищенного метода setPaConfig для исправления ошибки
 * компиляции. Оптимизирован порядок инициализации радио (setTCXO).
 */
 ...
 void sendFile(const uint8_t* data, size_t dataLen) {
   ...
   for (uint8_t i = 0; i < totalPackets; i++) {
     ...
     Serial.printf("\n[TX] Отправка пакета %d/%d... ", i + 1, totalPackets);
 ...
 void setup() {
   ...
   Serial.printf("\n=== LR1121 Reliable File Transfer v16.2 ===\n");
